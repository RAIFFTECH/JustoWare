...
class Detalles(LoginRequiredMixin, DetailView):
    model = ASOCIADOS
    form = CrearForm
    template_name = 'detalles_asociado.html'
...
class ImprimirPDF(View):
    def get(self, request, *args, **kwargs):
        ...
        asociados = ASOCIADOS.objects.all().order_by(
            'oficina', 'agno')
        ...
        datos_tabla = [["Oficina", "Año", "Meses", "IniAdu", "TotAdu",
                        "IniChi1", "TotChi1", "IniChi2", "TotChi2", "IniJur", "TotJur"]]

        for dato in asociados:
            datos_tabla.append([dato.oficina, dato.agno, dato.meses, dato.iniadu, dato.totadu,
                               dato.inichi1, dato.totchi1, dato.inichi2, dato.totchi2, dato.inijur, dato.totjur])
...
class ExportarView(View):
    ...
    def exportar_excel(self):
        ...
        headers = [field.name for field in ASOCIADOS._meta.fields]
        for col_num, header in enumerate(headers, 1):
            sheet.cell(row=1, column=col_num, value=header)

        # Añade datos a la hoja de cálculo
        asociados = ASOCIADOS.objects.all()
        for row_num, data in enumerate(asociados, start=2):
