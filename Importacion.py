...
from creditos_app.models import CREDITOS, CODEUDORES
...
def inicio():
    ...
    with open('c:/ajusto/csv/localidades.csv', 'r') as file:
        ...
        for row in csv_reader:
            ...
            if Localidad == None:
                Localidad = LOCALIDADES.objects.create(cliente = Cliente,
                    codigo = row['codigo'])
...
#  se busca la Calificacion a partir de un Puntaje Cartera de Consumo Con Libranza Entra el Puntaje y sale la Calificacion
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCCL',calificacion='A',pi_puntaje = 0.0174)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCCL',calificacion='B',pi_puntaje = 0.0337)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCCL',calificacion='C',pi_puntaje = 0.0479)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCCL',calificacion='D',pi_puntaje = 0.0812)
...
#  se busca la Calificacion a partir de un Puntaje Cartera de Consumo Sin Libranza  Entra el Puntaje y sale la Calificacion
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCSL',calificacion='A',pi_puntaje = 0.0559)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCSL',calificacion='B',pi_puntaje = 0.1066)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCSL',calificacion='C',pi_puntaje = 0.2199)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCSL',calificacion='D',pi_puntaje = 0.3690)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCSL',calificacion='E',pi_puntaje = 1)
#  se busca la Calificacion a partir de un Puntaje Cartera Comercial Persona Natural Entra el Puntaje y sale la Calificacion
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCPN',calificacion='A',pi_puntaje = 0.2340)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCPN',calificacion='B',pi_puntaje = 0.4291)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCPN',calificacion='C',pi_puntaje = 0.5905)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCPN',calificacion='D',pi_puntaje = 0.7571)
    PE_CALIF_RANGO.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCPN',calificacion='E',pi_puntaje = 1)
...
#  Cartera de Consumo Con Libranza Entra le Calificacion y sale la Porbabilidad de Incumplimieno PI en porcentaje
    ...
    PE_PI_CALIF.objects.create(cliente=Cliente,clase_coop='EAYC',modalidad = 'CCCL',calificacion='F',pi_porcent = 100.0)
...
#  Se introducen los coeficientes de los modelos
    PE_MODE_REFE.objects.create(cliente = Cliente,modalidad = 'CCCL',constante = -2.250040,coe_ea = -0.84440,coe_ap = -1.05730,coe_tc = 1.07150,coe_fe = 0.01390,
    ...
    PE_MODE_REFE.objects.create(cliente = Cliente,modalidad = 'CCSL',constante = -1.8017,coe_ea = -0.3758,coe_ap = 0.0,coe_tc = 0.0,coe_fe = 0.0,
...
def pagadores():
    ...
    with open('c:/ajusto/csv/s07entidades.csv', 'r') as file:
        csv_reader = csv.DictReader(file,delimiter=';')    
        for row in csv_reader:
            pagador = PAGADORES.objects.filter(cliente=Cliente,codigo=row['s07codent']).first()
            if pagador == None:
                pagador=PAGADORES.objects.create(cliente = Cliente,
...
def socios():
    ...
    with open('c:/ajusto/csv/s01socios.csv', 'r') as file:
        ...
        for row in csv_reader:
            ...
            pagador = PAGADORES.objects.filter(cliente=Cliente,codigo=row['s01codent']).first()
...
def detalle_econo():
    ...
    Cliente = CLIENTES.objects.filter(codigo='A').first()
    CentroCosto = CENTROCOSTOS.objects.filter(cliente=Cliente,codigo = 'A001').first()
    with open('c:/ajusto/csv/doctos_23.csv', 'r') as file:
        ...
        for row in csv_reader:
            ...
            if Cuenta == None:
                ...
                continue
            #DetalleEcono = DETALLE_ECONO.objects.filter(hecho_econo=Comprobante,cuenta=Cuenta,tercero=Tercero).first()
            #if DetalleEcono == None:
            DetalleEcono = DETALLE_ECONO.objects.create(hecho_econo=Comprobante,cuenta=Cuenta,tercero=Tercero)
    ...
    with open('c:/ajusto/csv/s06movaho.csv', 'r') as file:
        ...
        for row in csv_reader:
            ...
            if Doc == None:         # No Hay Documento
                continue
            Com = HECHO_ECONO.objects.filter(docto_conta = Doc,numero = int(row['s06documen'])).first()
    ...
    with open('c:/ajusto/csv/xMovIntCtaAho.csv', 'r') as file:
        ...
        for row in csv_reader:
            ...
            if Socio == None:
                ...
                continue
            CtaCon = PLAN_CTAS.objects.filter(cliente = Cliente,per_con=row['per_con'],cod_cta=row['cod_cta']).first()
            if CtaCon == None:
                print('No Existe Cta Con ',row['cod_cta'],'  en el Periodo  ',row['per_con'])
                continue
            Docto = DOCTO_CONTA.objects.filter(oficina = Oficina,per_con = row['per_con'],codigo = 131).first()
            ...
            if row['cod_cta'][:1] != '2':
                ...
                if CtaAho == None:
                    continue
                DetPro = DETALLE_PROD.objects.filter(hecho_econo = HecEco,producto = 'AH',concepto = 'AHO',subcuenta = CtaAho.num_cta,centro_costo = CentroCosto).first()
                if DetPro == None:
                    DetPro = DETALLE_PROD.objects.create(hecho_econo = HecEco,producto = 'AH',concepto = 'AHO',subcuenta = CtaAho.num_cta,centro_costo = CentroCosto)
...
def cat_des_dia_cre():
    ...
    with open('c:/ajusto/csv/s19catdescre.csv', 'r') as file:
        csv_reader = csv.DictReader(file,delimiter=';')
        for row in csv_reader:
            CatDesDiaCre = CAT_DES_DIA_CRE.objects.filter(cliente=Cliente,codigo = ord(row['s19coddes']),categoria = row['s19codcat']).first()
            if CatDesDiaCre == None:
                CatDesDiaCre = CAT_DES_DIA_CRE.objects.create(cliente=Cliente,codigo = ord(row['s19coddes']),categoria = row['s19codcat'])
...
def codeudores():
    ...
    with open('c:/ajusto/csv/s10codeudor.csv', 'r') as file:
        ...
        for row in csv_reader:
            ...
            if Credito == None:
                continue
            Tercero = TERCEROS.objects.filter(cliente=Cliente,doc_ide=row['s10nit']).first()
            if Tercero == None:
                continue
            Codeudor = CODEUDORES.objects.filter(oficina=Oficina,credito=Credito,tercero=Tercero).first()
            if Codeudor == None:
                Codeudor = CODEUDORES.objects.create(oficina=Oficina,credito=Credito,tercero=Tercero)
            Codeudor.save()
    print('          ...  ',datetime.now())
...
def init():
    ...
    # terceros()  # 2. SOLO LOS REGISTRADOS EN DINAMICA
    # pagadores() # 3.
    # plan_ctas() # 4.
    # conceptos() # 5.
    # docto_conta() # 6.
