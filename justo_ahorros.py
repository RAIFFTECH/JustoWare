...
class ahorros():
    ...
    def interes_diario(self):
        ...
        Ctas = justo.CTAS_AHORRO.objects.filter(oficina=self.Oficina, est_cta='A').filter(
            Q(num_cta__startswith='03') | Q(num_cta__startswith='05'))
        Doc = justo.DOCTO_CONTA.objects.filter(
        ...
        if HecEco == None:
            HecEco = justo.HECHO_ECONO.objects.create(
                docto_conta=Doc, numero=self.fecha_liq.timetuple().tm_yday)
        ...
        for Cta in Ctas:
            ...
            if Cta.num_cta[0:2] != '04':
                LinAho = justo.LINEAS_AHORRO.objects.filter(
                    cliente=self.Oficina.cliente, cod_lin_aho=Cta.num_cta[0:2]).first()
                TasLin = justo.TAS_LIN_AHO.objects.filter(
                    lin_aho=LinAho, fecha_inicial__lte=self.fecha_liq, fecha_final__gte=self.fecha_liq).first()
                TasRF = justo.RET_FUE_AHO.objects.filter(
                    lin_aho=LinAho, fecha_inicial__lte=self.fecha_liq, fecha_final__gte=self.fecha_liq).first()
                xTasIntDia = TasLin.tiae / 100
                xTasNomDia = round(((1 + xTasIntDia) ** (1 / 365)) - 1, 6)
                xIntDia = round(self.saldo_fecha*xTasNomDia, 0)
                if xIntDia < 0:
                    xDesRF = round(-xIntDia*TasRF.tas_liq_rf,
                                   0) if TasRF.bas_liq_int <= -xIntDia else 0
                    DiaInt = justo.INT_DIA_AHO.objects.filter(
                        oficina=self.Oficina, cta_aho=Cta, num_cta=Cta.num_cta, dia_mes=self.fecha_liq.day).first()
                    if DiaInt == None:
                        DiaInt = justo.INT_DIA_AHO.objects.create(
                            oficina=self.Oficina, cta_aho=Cta, num_cta=Cta.num_cta, dia_mes=self.fecha_liq.day)
                    ...
                    DiaInt.save()
        LinAho = justo.LINEAS_AHORRO.objects.filter(
            cliente=self.Oficina.cliente, cod_lin_aho='04').first()
        TasRF = justo.RET_FUE_AHO.objects.filter(
            lin_aho=LinAho, fecha_inicial__lte=self.fecha_liq, fecha_final__gte=self.fecha_liq).first()
        ...
        for CdatFec in CdatFecs:
            if CdatFec.cta_aho.oficina != self.Oficina:
                continue
            Cdat = justo.CTA_CDAT.objects.filter(
                cta_aho=CdatFec.cta_aho).first()
            ...
            DetPro = justo.DETALLE_PROD.objects.filter(
                hecho_econo=HecEco, producto='AH', concepto='AHO', subcuenta=Cdat.cta_int_ret).first()
            if DetPro == None:
                DetPro = justo.DETALLE_PROD.objects.create(
                    hecho_econo=HecEco, producto='AH', concepto='AHO', subcuenta=Cdat.cta_int_ret)
            DetPro.valor = xIntPorApl + xInt - xRetFue - xRetFuePorApl
            DetPro.save()
            justo.CTA_CDAT_LIQ.objects.filter(
                cta_amp=CdatFec, fecha__gte=self.fecha_liq).delete()
            MovCdaLiq = justo.CTA_CDAT_LIQ.objects.filter(
                cta_aho=CdatFec.cta_aho, cta_amp=CdatFec, fecha=self.fecha_liq, tip_liq='C').first()
            if MovCdaLiq == None:
                MovCdaLiq = justo.CTA_CDAT_LIQ.objects.create(
                    cta_aho=CdatFec.cta_aho, cta_amp=CdatFec, fecha=self.fecha_liq, tip_liq='C')
            ...
            MovCdaLiq.save()
            MovCdaLiq1 = justo.CTA_CDAT_LIQ.objects.filter(
                cta_aho=CdatFec.cta_aho, cta_amp=CdatFec, fecha=self.fecha_liq, tip_liq='P').first()
            if MovCdaLiq1 == None:
                MovCdaLiq1 = justo.CTA_CDAT_LIQ.objects.create(
                    cta_aho=CdatFec.cta_aho, cta_amp=CdatFec, fecha=self.fecha_liq, tip_liq='P')
            ...
            CtaAho = justo.CTAS_AHORRO.objects.filter(
                oficina=self.Oficina, id=Cdat.cta_aho_id).first()
            ImpCon = justo.IMP_CON_LIN_AHO.objects.filter(
            ...
            if xIntPorApl > 0:
                PlaCtaInt = justo.PLAN_CTAS.objects.filter(
                    cliente=self.Oficina.cliente, per_con=self.fecha_liq.year, cod_cta=xCtaInt).first()
                DetEcoInt = justo.DETALLE_ECONO.objects.filter(
                    hecho_econo=HecEco, cuenta=PlaCtaInt, tercero=CtaAho.asociado.tercero).first()
                if DetEcoInt == None:
                    DetEcoInt = justo.DETALLE_ECONO.objects.create(
                        hecho_econo=HecEco, cuenta=PlaCtaInt, tercero=CtaAho.asociado.tercero)
                ...
                DetEcoInt.valor_2 = 0
                DetEcoInt.save()
                PlaCtaPag = justo.PLAN_CTAS.objects.filter(
                    cliente=self.Oficina.cliente, per_con=self.fecha_liq.year, cod_cta=xCtaPXCda).first()
                DetEcoPag = justo.DETALLE_ECONO.objects.filter(
                    hecho_econo=HecEco, cuenta=PlaCtaPag, tercero=CtaAho.asociado.tercero).first()
                if DetEcoPag == None:
                    DetEcoPag = justo.DETALLE_ECONO.objects.create(
                        hecho_econo=HecEco, cuenta=PlaCtaPag, tercero=CtaAho.asociado.tercero)
            ...
            if xIntPorApl + xInt - xRetFue - xRetFuePorApl - xUltRetFue != 0:
                ...
                DetEcoPag.valor_2 = 0
                DetEcoPag.save()
                PlaCtaAho = justo.PLAN_CTAS.objects.filter(
                    cliente=self.Oficina.cliente, per_con=self.fecha_liq.year, cod_cta=xCtaAhoDes).first()
                DetEcoAho = justo.DETALLE_ECONO.objects.filter(
                    hecho_econo=HecEco, cuenta=PlaCtaAho, tercero=CtaAho.asociado.tercero).first()
                if DetEcoAho == None:
                    DetEcoAho = justo.DETALLE_ECONO.objects.create(
                        hecho_econo=HecEco, cuenta=PlaCtaAho, tercero=CtaAho.asociado.tercero)
    ...
    def interes_mensual(self):
        ...
        Ctas = justo.CTAS_AHORRO.objects.filter(oficina=self.Oficina, est_cta='A').filter(Q(num_cta__startswith='01') | Q(
        ...
        for Cta in Ctas:
            ...
            if xIntMes < 0:
                ...
                if -xIntMes > TasRF.bas_liq_int * xDiaMes:
                    ...
                    DetEcoMovRet = justo.DETALLE_ECONO.objects.filter(
                        hecho_econo=HecEco, cuenta=PlaCtaCta, tercero=Cta.asociado.tercero).first()
                    if DetEcoMovRet == None:
                        DetEcoMovRet = justo.DETALLE_ECONO.objects.create(
                            hecho_econo=HecEco, cuenta=PlaCtaCta, tercero=Cta.asociado.tercero)
                    DetEcoMov.detalle_prod = DetPro
                    DetEcoMov.item_concepto = 'IntCor'
        ...
        IntDias = justo.INT_DIA_AHO.objects.filter(aplicado='N', fecha__month=self.fecha_liq.month, oficina=Oficina).values(
