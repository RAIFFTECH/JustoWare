...
class CLIENTES(models.Model):
    ...
    clase_coop = models.CharField(max_length=8,choices=CLASE_COOP,default = 'EAYC')
...
def asignar_numero_HECHO_ECONO(sender, instance, created, **kwargs):
    if created and instance.numero is None:
        ultimo_numero = HECHO_ECONO.objects.filter(docto_conta=instance.docto_conta).order_by('-numero').first()
...
class CREDITOS(models.Model):
    ...
    for_pag = models.CharField(max_length=1, choices=OPC_CRE_FOR_PAG,null = True)
    tip_gar = models.CharField(max_length=2, choices=OPC_GARANTIA,default = '15')
...
class PE_CALIF_RANGO(models.Model):    # Entrega la Calificacion a Partir de un Puntaje por rANGOS
    cliente = models.ForeignKey(CLIENTES, on_delete=models.CASCADE)
