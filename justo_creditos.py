...
def cargue_mov_cre(iCodCre, valores):
    ...
    Credito = justo.CREDITOS.objects.filter(
        oficina=Oficina, cod_cre=iCodCre).first()
    xTotSal = 0
    xNumCre = 0
    ...
    queryset2 = justo.DETALLE_PROD.objects.filter(oficina=Oficina, producto='CR', subcuenta=Credito.cod_cre) \
        ...
        .annotate(
            cuota=Value(0),
            tipmov=Case(
                *[When(concepto=concept, then=Value(value))
                  for concept, value in TIPOS_MOV_CRE.items()],
                output_field=CharField()
            ),
            kapital=Coalesce(Sum(Case(When(detalle_econo__item_concepto='Kapita', then=-F(
                'detalle_econo__valor_2')+F('detalle_econo__valor_1')))), Value(0.0)),
            intcor=Coalesce(Sum(Case(When(detalle_econo__item_concepto='IntCor', then=-F(
                'detalle_econo__valor_2')+F('detalle_econo__valor_1')))), Value(0.0)),
            intmor=Coalesce(Sum(Case(When(detalle_econo__item_concepto='IntMor', then=-F(
                'detalle_econo__valor_2')+F('detalle_econo__valor_1')))), Value(0.0)),
            polseg=Coalesce(Sum(Case(When(detalle_econo__item_concepto='PolSeg', then=-F(
                'detalle_econo__valor_2')+F('detalle_econo__valor_1')))), Value(0.0)),
            despp=Coalesce(Sum(Case(When(detalle_econo__item_concepto='DesPP', then=-F(
                'detalle_econo__valor_2')+F('detalle_econo__valor_1')))), Value(0.0)),
            acreed=Coalesce(Sum(Case(When(detalle_econo__item_concepto='Acreed', then=-F(
                'detalle_econo__valor_2')+F('detalle_econo__valor_1')))), Value(0.0))
    )
...
def calculo_cuota(ikapital, iTIEA, iTIDIC, iPerio, iNumCuo, iFecDes, iFecPagIni, iNumCuoGra=0, iTIDPS=0, iIntCorAnt=0):
    ...
    while True:
        ...
        for xPer in range(1, iNumCuo+iNumCuoGra+1):
            ...
            xCapPer = round(xValCuo-xIntApl-xIntPer if xPer >
                            iNumCuoGra and xValCuo-xIntApl-xIntPer > 0 else 0)
            xNueCapPer = xCapPer
            # xCapPer= xCapPer + si tiene cuotas extras
            xIntApl = xIntApl + (xIntPer if xPer < iNumCuoGra else 0)
            xIntApl = xIntApl + (xValCuo - xNueCapPer -
                                 xIntPer if xIntPorApl > 0 else 0)
...
class Liquida_cre:
    ...
    def _calculos_generales(self):
        ...
        for reg in self.lista_mov:
            ...
            #    self.cap_ini = reg['kapital']
                continue
            self.sal_cap_tot = self.sal_cap_tot + (reg['kapital'] if (
                reg['fecha'] - self.fecha_focal).days <= 0 or reg['tipmov'] == '0' or reg['tipmov'] == '1' else 0)
            xCapPag = xCapPag + (reg['kapital'] if reg['fecha']
                                 <= self.fecha_focal and reg['tipmov'] != '1' else 0)
            self.sal_cap_dia = self.sal_cap_dia + \
                (reg['kapital'] if reg['fecha'] <= self.fecha_focal else 0)
    ...
    def calculo_periodo(self):
        self.int_pag_per = 0
        self.int_condo = 0
        lista_periodo = [objeto for objeto in self.lista_mov if objeto['fecha'].year ==
                         self.fecha_focal.year and objeto['fecha'].month == self.fecha_focal.month and objeto['tipmov'] > '3']
    ...
    def liq_al_dia(self, ifecha_focal=datetime.now().date(), recarga=False):
        ...
        xSalCuoPag = 0
        lista_al_dia = [objeto for objeto in self.lista_mov if objeto['fecha']
                        <= self.fecha_focal and objeto['tipmov'] != 'A']
        ...
        for reg in lista_al_dia:
            if reg['tipmov'] == '3' and reg['fecha'] > xFecUltCau and reg['fecha'] >= xFecUltPag and xFecUltPag <= self.fecha_focal and xFecUltPag >= xFecUltCau:
                oIntAjuAboCap = oIntAjuAboCap - \
                    (reg['int_cor'] if reg['intcor'] < 0 else 0)
            if reg['fecha'] <= xFecUltCau:
                xSalProDia = xSalProDia - \
                    (reg['kapital'] if reg['tipmov'] == '1' else 0)
                xSalReaDia = xSalReaDia + \
                    (reg['kapital'] if reg['tipmov'] >= '4' else 0)
                if reg['fecha'] == xFecUltCau and reg['tipmov'] == '4':
                    xYaAjuIntAnt = True
                else:
                    oIntPagPosDia = oIntPagPosDia - \
                        (reg['intcor'] if reg['tipmov'] else 0)
            if reg['tipmov'] >= '5':
                xSalReaAnt = xSalRea
                xSalRea = xSalRea + reg['kapital']
                if reg['fecha'] > xFecUltCau:
                    self.int_cau_fra = self.int_cau_fra + int((xSalPro if xSalPro <= xSalRea else (
                        xSalPro if xSalPro <= xSalReaAnt else xSalReaAnt))*(reg['fecha']-xFecAntMov).days*self.tas_ic_dia)
                ...
                if reg['fecha'] > xFecCauCapNeg:
                    if xSalCuoPag > 0:
                        if reg['fecha'] <= xFecRefMor and xFecRefMor <= self.fecha_focal:
                            xDiaMor = ((self.fecha_focal-xFecRefMor).days if (
                                xFecRefMor-reg['fecha']).days > xDiaConIntMor else (self.fecha_focal-reg['fecha']).days)
                            xDiaMor = xDiaMor if (
                                self.fecha_focal-reg['fecha']).days > xDiaConIntMor else 0
                        else:
                            xDiaMor = (self.fecha_focal - reg['fecha']).days
                            xDiaMor = xDiaMor if xDiaMor > xDiaConIntMor else 0
                        self.int_mor_cau = self.int_mor_cau + \
                            round((reg['kapital'] if xSalCuoPag > reg['kapital']
                                  else xSalCuoPag)*xDiaMor*self.tas_im_anual/36525, 0)
        if (self.fecha_focal - xFecAntMov).days > 0:
            self.int_cau_fra = self.int_cau_fra + \
                int((xSalPro if xSalPro < xSalRea else xSalRea) *
                    (self.fecha_focal-xFecAntMov).days*self.tas_ic_dia)
        ...
        for reg in self.lista_mov:
            if reg['tipmov'] == '1':
                if xBasCapAju > 0:
                    ...
                    self.int_aju_pa = self.int_aju_pa + \
                        round(xBasCap*(reg['fecha'] -
                              xFecUltCau).days*self.tas_ic_dia, 0)
        ...
        lista_final = [objeto for objeto in self.lista_mov if objeto['fecha']
                       > self.fecha_focal and objeto['tipmov'] == '1']
        xSig = 0
        for reg in lista_final:
            if xSig == 0:
                ...
                self.min_pag_aboca = self.int_cor_a_pag + \
                    self.pol_seg_a_pag + self.int_mor_a_pag + self.int_cau_fra
                self.min_pag_aboca = self.min_pag_aboca + \
                    (self.sal_cap_dia if self.sal_cap_tot - self.sal_cap_dia <
                     xMaxSaldo else xMaxSaldo - self.sal_cap_tot)
            elif xSig == 1:
                self.fec_sig_pag2 = reg['fecha']
                xDia2 = (self.fec_sig_pag2 - self.fecha_focal).days
                xMaxSaldo = self.val_cuo / (self.tas_ic_dia*xDia2)
                self.min_pag_abocu = self.int_cor_a_pag + \
                    self.pol_seg_a_pag + self.int_mor_a_pag + self.int_cau_fra
                self.min_pag_abocu = self.min_pag_abocu + \
                    (self.sal_cap_dia if self.sal_cap_tot - self.sal_cap_dia <
                     xMaxSaldo else xMaxSaldo - self.sal_cap_tot)
            else:
                break
    ...
    def liq_por_cuotas(self, iAltura):
        ...
        for reg in lista_al_dia:
            if reg['tipmov'] != '1':
                ...
                xMorPag = xMorPag - \
                    (reg['intmor'] if reg['tipmov'] >= '5' else 0)
                xMorCau = xMorCau + \
                    (reg['intmor'] if reg['tipmov'] == '2' else 0)
        ...
        if xFecUltPag <= self.fecha_focal:
            for reg in lista_al_dia:
                if reg['tipmov'] == '1':
                    ...
                    if xCapMor > 0:
                        xDiaMor1 = ((self.fecha_focal if xFecUltPag >
                                    self.fecha_focal else xFecUltCau) - reg['fecha']).days
                        xDiaMor1 = xDiaMor1 if xDiaMor1 > xDiaConIntMor else 0
                        xMorAnt = xMorAnt + \
                            round(reg['kapital'] if xCapMor > reg['kapital']
                                  else xCapMor*xDiaMor1*self.tas_im_anual/36525, 0)
            ...
            if reg['tipmov'] == '1':
                ...
                if xCapCau >= xCapPag:
                    ...
                    if reg['fecha'] <= xFecUltPag:
                        xDiaMor1 = ((self.fecha_focal if xFecUltPag >
                                    self.fecha_focal else xFecUltPag) - reg['fecha']).days
                        xDiaMor1 = xDiaMor1 if xDiaMor1 > xDiaConIntMor else 0
                        xDiaMor2 = (
                            self.fecha_focal - xFecUltPag).days if oCapital > reg['kapital'] else 0
                        xMorCauAnt = round(
                            (reg['kapital'] if oCapital > reg['kapital'] else oCapital)*xDiaMor1*self.tas_im_anual/36525, 0)
                        if xMorYaPag > 0:
                            if xMorYaPag > xMorCauAnt:
                                ...
                                xMorYaPag = 0
                        xMorCauPer = xMorCauAnt + \
                            round((reg['kapital'] if oCapital > reg['kapital']
                                  else oCapital)*xDiaMor2*self.tas_im_anual/36525, 0)
                        xDiaMor = (self.fecha_focal - reg['fecha']).days if xFecUltPag > self.fecha_focal else (
                            self.fecha_focal - xFecUltPag).days
                        xMorCauCuo = xMorCauCuo + xMorCauPer
                    else:
                        xDiaMor = (self.fecha_focal - reg['fecha']).days if (
                            self.fecha_focal - reg['fecha']).days > xDiaConIntMor else 0
                        xMorCauPer = round(
                            (reg['kapital'] if oCapital > reg['kapital'] else oCapital)*xDiaMor*self.tas_im_anual/36525, 0)
                        xMorCauCuo = xMorCauCuo + xMorCauPer
        ...
        if Anticipo:
            lista_al_dia = [
                objeto for objeto in self.lista_mov if objeto['fecha'] > self.fecha_focal]
            for reg in lista_al_dia:
                ...
                if reg['tipmov'] == '1':
                    ...
                    xIntPer = oIntCor if oIntCor < reg['intcor'] and oIntCor >= 0 else (
                        0 if oIntCor < 0 else reg['intcor'])
                    xFacInt = ((reg['fecha']-self.fecha_focal).days if (
                        reg['fecha']-self.fecha_focal).days > 0 else 0)*self.tas_ic_dia
                    xConIntAnt = round(xCapPer*self.tas_ic_dia, 0)*(
                        (reg['fecha']-self.fecha_focal).days if (reg['fecha']-self.fecha_focal).days > 0 else 0)
    ...
    def distri_pago_cuota(self, iValPorDis):
        ...
        xDiaConIntMor = 0
        xFecCer = self.fec_des
        xTasIntPer = round((round(self.tas_ic_ea/100, 4)+1)
                           ** (1/self.per_ano), 4)
        xTasDesIntPer = 0
                ...
                if not (reg['fecha'] + timedelta(days=35) > xFecCauCapNeg and reg['fecha'] < xFecCauCapNeg):
                    if reg['fecha'] < xFecUltPag:
                        if xFecUltPag > self.fecha_focal:
                            ...
                            xMoorCauNvo = round(
                                (reg['fecha'] if xCapPer > reg['fecha'] else xCapPer) * xDiaMor2*(self.tas_im_anual/36525), 0)
                        else:
                            ...
                            if xAntAboCap > 0:
                                xMorCauAnt = round(
                                    (reg['kapital'] if xAntAboCap > reg['kapital'] else xAntAboCap)*xDiaMor1*(self.tas_im_anual/36525), 0)
                            else:
                                xMorCauAnt = 0
                            xDiaMor2 = (self.fecha_focal - xFecUltPag).days + \
                                (xDiaMorIni if xDiaMorIni != xDiaMor1 else 0)
                            xDiaMor2 = xDiaMor2 if xDiaMor2 > xDiaConIntMor else 0
                            xMorCauNvo = round(
                                (reg['kapital'] if xCapPer > reg['kapital'] else xCapPer)*xDiaMor2*(self.tas_im_anual/36525), 0)
                        ...
                        xDiaMor = xDiaMor if xDiaMor > xDiaConIntMor else 0
                        xMorCauPer = round(
                            (reg['kapital'] if xCapPer > reg['kapital'] else xCapPer)*xDiaMor*(self.tas_im_anual/36525), 0)
                ...
                for rega in lista_al_dia[lista_al_dia.index(reg):]:
                    ...
                    if xPrimero == False:
                        ...
                        if rega['fecha'] <= xFecUltPag:
                            if xFecUltPag > self.fecha_focal:
                                xDiaMor1 = 0
                                xDiaMor2 = (self.fecha_focal -
                                            rega['fecha']).days
                            else:
                                xDiaMor1 = (xFecUltPag - rega['fecha']).days
                                xDiaMor2 = (self.fecha_focal - xFecUltPag).days + \
                                    (xDiaMor1 if xDiaMor1 <= xDiaConIntMor else 0)
                            xDiaMor1 = xDiaMor1 if xDiaMor1 > xDiaConIntMor else 0
                            if xAntAboCap > 0:
                                xMorCauAnt = round(
                                    (rega['kapital'] if xAntAboCap > rega['kapital'] else xAntAboCap)*xDiaMor1*(self.tas_im_anual/36525), 0)
                            else:
                                xMorCauAnt = 0
                            if xMorYaPag > 0:
                                if xMorYaPag > xMorCauAnt:
                            ...
                            xMorFra = round((rega['kapital'] if xCapPer > rega['kapital'] and rega['kapital'] > 0 else (
                                xCapPer if xCapPer <= rega['kapital'] else 0))*xDiaMor2*(self.tas_im_anual/36525), 0)
                            xMorCauPer = xMorCauPer + xMorFra
                            oCauIntMor = oCauIntMor + xMorFra
                        else:
                            xDiaMor = (self.fecha_focal - rega['fecha']).days
                            xDiaMor = xDiaMor if xDiaMor > xDiaConIntMor else 0
                            xMorCauPer = round((rega['kapital'] if xAboCap > rega['kapital'] and rega['kapital'] > 0 else (
                                xAboCap if xAboCap <= rega['kapital'] else 0))*xDiaMor*(self.tas_im_anual/36525), 0)
                            oCauIntMor = oCauIntMor + xMorCauPer
                    xPrimero = False
                    if xPolSegPer > 0:
...
#  causar la mora total si paga algo pero no queda al dia avanza al sigiente regisdtro
                for regb in lista_al_dia[lista_al_dia.index(rega) + 1:]:
                    ...
                    if regb['fecha'] <= xFecUltPag and xAboCap > 0:
                        xDiaMor = (self.fecha_focal - regb['fecha']).days if xFecUltPag > self.fecha_focal else (
                            self.fecha_focal - xFecUltPag).days
                        xDiaMor = xDiaMor if xDiaMor > xDiaConIntMor else 0
                        oCauIntMor = oCauIntMor + \
                            round((rega['kapital'] if xAboCap > rega['kapital'] else xAboCap)
                                  * xDiaMor if xDiaMor > 0 else 0*(self.tas_im_anual/36525), 0)
                    else:
                        xDiaMor = (self.fecha_focal - regb['fecha']).days if (
                            self.fecha_focal - regb['fecha']).days > 0 else 0
                        xDiaMor = xDiaMor if xDiaMor > xDiaConIntMor else 0
                        oCauIntMor = oCauIntMor + \
                            round((rega['kapital'] if xAboCap > rega['kapital']
                                  else xAboCap)*xDiaMor*(self.tas_im_anual/36525), 0)
                break
        lista_posterior = [objeto for objeto in self.lista_mov if objeto['fecha']
                           > self.fecha_focal and objeto['tipmov'] == '1']
        for reg2 in lista_posterior:
            if xSalPorDis > 0:
                ...
                xCapPer = xCapPer if xCapPer > 0 else 0
                xIntPer = xAboIntCor if xAboIntCor < reg2[
                    'intcor'] and xAboIntCor >= 0 else reg2['intcor'] if xAboIntCor >= 0 else 0
                xFacInt = ((reg2['fecha'] - self.fecha_focal).days if (
                    reg2['fecha'] - self.fecha_focal).days > 0 else 0)*self.tas_ic_dia
                xConIntAnt = round(xCapPer * self.tas_ic_dia, 0) * (
                    reg2['fecha'] - self.fecha_focal).days if (reg2['fecha'] - self.fecha_focal).days > 0 else 0
                ...
                if xTotPer <= xSalPorDis and xTotPer >= 0:
                    ...
                    if xSalPorDis > xIntPer:
                        xCap = int((xSalPorDis - xIntPer*(1-xTasDesIntPer /
                                   xTasIntPer))/(1-xFacInt*(1-xTasDesIntPer/xTasIntPer)))
    ...
    def distri_pago_abo(self, iValorPorDis, idet_pro=None):
        ...
        xFec_mod = None
        if idet_pro != None:
            reg_pag_act = justo.DETALLE_PROD.objects.filter(
                id=idet_pro).first()
            if reg_pag_act != None:
                xValPagAct = -reg_pag_act.valor
                reg_hecho = justo.HECHO_ECONO.objects.filter(
                    id=reg_pag_act.hecho_econo).first()
                xfec_mod = reg_hecho.fecha
            if xValPagAct != iValorPorDis or xFec_mod != self.fecha_focal or reg_pag_act.concepto == 'ABOCU':
                return 2
            regis = [fila for fila in self.lista_mov if fila['fecha']
                     == self.fecha_focal and fila['tip_mov'] == '7']
        ...
        xIntDia = xSalIntDia if xSalIntDia > 0 else - \
            xIntPagTot if xSalIntDia + xIntPagTot < 0 else xSalIntDia
    ...
    def aplicar_pago(self, iValPorDis):
        Cliente = justo.CLIENTES.objects.filter(codigo='A').first()
        CenCos = justo.CENTROCOSTOS.objects.filter(
            cliente=Cliente, codigo='A001').first()
        Oficina = justo.OFICINAS.objects.filter(codigo='A0001').first()
        Credito = justo.CREDITOS.objects.filter(
            oficina=Oficina, cod_cre=self.cod_cre).first()
        ImpConCre = justo.IMP_CON_CRE.objects.filter(
            id=Credito.imputacion_id).first()
        ...
        if ImpConCre == None:
            ...
            return
        xValPag = -(self.capital_a_pag + self.int_cor_a_pag +
                    self.pol_seg_a_pag + self.int_mor_a_pag + self.acreedor_a_pag)
        if xValPag == 0 or xValPag != iValPorDis:
            return
        DocCon = justo.DOCTO_CONTA.objects.filter(
            oficina=Oficina, per_con=self.fecha_focal.year, codigo=1).first()
        HecEco = justo.HECHO_ECONO.objects.filter(
            docto_conta=DocCon, numero=1).first()
        if HecEco == None:
            HecEco = justo.HECHO_ECONO.objects.create(
                docto_conta=DocCon, numero=None, fecha=self.fecha_focal, descripcion='Aboca Prueba')
        DetPro = justo.DETALLE_PROD.objects.filter(
            hecho_econo=HecEco, oficina=Oficina, centro_costo=CenCos, producto='CR', concepto=self.tip_pag, subcuenta=self.cod_cre).first()
        if DetPro == None:
            DetPro = justo.DETALLE_PROD.objects.create(
                hecho_econo=HecEco, oficina=Oficina, centro_costo=CenCos, producto='CR', concepto=self.tip_pag, subcuenta=self.cod_cre, valor=-xValPag)
        if self.capital_a_pag != 0:
            PlaCta = justo.PLAN_CTAS.objects.filter(
                cliente=Cliente, per_con=DocCon.per_con, cod_cta=ImpConCre.kpte_cap).first()
            DetEco = justo.DETALLE_ECONO.objects.filter(
                hecho_econo=HecEco, detalle_prod=DetPro, cuenta=PlaCta, tercero=Ter).first()
            if DetEco == None:
                DetEco = justo.DETALLE_ECONO.objects.create(
                    hecho_econo=HecEco, detalle_prod=DetPro, cuenta=PlaCta, tercero=Ter)
            if self.capital_a_pag < 0:
        ...
        if self.int_cor_a_pag != 0:
            PlaCta = justo.PLAN_CTAS.objects.filter(
                cliente=Cliente, per_con=DocCon.per_con, cod_cta=ImpConCre.kpte_ic).first()
            DetEco = justo.DETALLE_ECONO.objects.filter(
                hecho_econo=HecEco, detalle_prod=DetPro, cuenta=PlaCta, tercero=Ter).first()
            if DetEco == None:
                DetEco = justo.DETALLE_ECONO.objects.create(
        ...
        if self.int_mor_a_pag != 0:
            PlaCta = justo.PLAN_CTAS.objects.filter(
                cliente=Cliente, per_con=DocCon.per_con, cod_cta='41504001').first()
            DetEco = justo.DETALLE_ECONO.objects.filter(
                hecho_econo=HecEco, detalle_prod=DetPro, cuenta=PlaCta, tercero=Ter).first()
            if DetEco == None:
                DetEco = justo.DETALLE_ECONO.objects.create(
        ...
        if self.int_mor_cau != 0:  # Faltan mas Condiciones
            CamCre = justo.CAMBIOS_CRE.objects.filter(
                det_pro=DetPro, tip_cam='2').first()
            if CamCre == None:
                CamCre = justo.CAMBIOS_CRE.objects.create(
                    det_pro=DetPro, tip_cam='2')
        ...
        lista_causa = [
            objeto for objeto in self.lista_mov if objeto['tipmov'] == '1']
        ...
        for reg in lista_al_dia:
            if reg['tipmov'] != 'A':
                ...
                xSalCap = xSalCap + \
                    (reg['kapital'] if reg['kapital'] < 0 else 0)
        ...
        for reg in lista_final:  # se re aplican causaciones futuras
            if xSalCap > 0:
                xIntMes = round(xSalCap * self.tas_ic_dia, 0) * \
                    (reg.fecha - xFecAnt).days
    ...
    def eliminar_pago(self, iDetPro):
        ...
        if DetPro.concepto == 'ABOCA' or DetPro.concepto == 'ABOCU' or DetPro.concepto == 'CASTI':
            CauCres = justo.CREDITOS_CAUSA.objects.filter(
                comprobante_id=DetPro, cod_cre=self.cod_cre, oficina=Oficina)
            if CauCres == None:
                return
            CauCreRets = justo.CREDITOS_CAUSA.objects.filter(
                comprobante_id=None, cod_cre=self.cod_cre, oficina=Oficina)
            ...
            for CauCre in CauCres:
                ...
                CauCre.save()
        HecEco = justo.HECHO_ECONO.objects.filter(
            id=DetPro.hecho_econo_id).first()
        DetEcos = justo.DETALLE_ECONO.objects.filter(
            hecho_econo=HecEco, detalle_prod=DetPro)
    ...
    def Exportar_mov(self):
        ...
        # Ajusta el nombre de la hoja según tu caso
...
class AsiCon:
    ...
    def actualizar(self, CodCta, Nit, Tipo, Detalle, Debito, Credito):
        ...
        if reg_existe:
            reg_existe.Detalle = reg_existe.Detalle + \
                ('' if Detalle in reg_existe.Detalle else ';'+Detalle)
            reg_existe.Debito = reg_existe.Debito + Debito
            reg_existe.Credito = reg_existe.Credito + Credito
        else:
            # Crear un nuevo registro si no existe
            nuevo_registro = AsiConRow(
                CodCta, Nit, Tipo, Detalle, Debito, Credito)
    ...
    def eliminar_iguales(self):
        ...
        self.rows = [
            registro for registro in self.rows if registro.Debito != registro.Credito]
...
class Reclasificacion:
    ...
    def Creacion_tablas(self, ifecha, Oficina):
        Cliente = justo.CLIENTES.objects.filter(codigo='A').first()
        # Elimina la Reclasificacion del mes para volver a empezar
        justo.CATE_INTE.objects.filter(
            oficina=Oficina, fecha=self.fecha_rec).delete()
    #   Colocar Castigo en la causacion anterior si durante el periodo se presento un castigo o una condonacion
        ...
        if CreMovKC != None:
            for reg in CreMovKC:
                DetPro = justo.DETALLE_PROD.objects.filter(
                    id=reg.det_pro_id).first()
                CreHis = justo.CARTE_CAT_HIS.objects.filter(
                    oficina=Oficina, fecha=self.fecha_ant_rec, cod_cre=DetPro.subcuenta).first()
                xCreCas = ' '
                ...
                CreHis.save()
        # se marcan los registros para luego borrarlos si no se reinicio
        justo.CARTE_CAT_HIS.objects.filter(
            oficina=Oficina, fecha=self.fecha_rec).delete()
        Creditos = justo.CREDITOS.objects.filter(oficina=Oficina).exclude(
            estado='H')  # se recorren todos los creditos
        nr = 0
        for Credito in Creditos:
            ...
            if liq_cre.sal_cap_tot <= 0:
                if not (Credito.fec_des.year == ifecha.year and Credito.fec_des.month == ifecha.month):
                    CarCatAnt = justo.CARTE_CAT_HIS.objects.filter(
                        oficina=Oficina, fecha=self.fecha_ant_rec, cod_cre=Credito.cod_cre).first()
                    if CarCatAnt == None:
                        continue
        ...
        #   Se crea o se modifica LA FOTO DE LA FECHA
            xCat = xCat if not (
                Credito.fec_des.year == ifecha.year and Credito.fec_des.month == ifecha.month) else 'A'
            CarCatHis = justo.CARTE_CAT_HIS.objects.filter(
                oficina=Oficina, fecha=self.fecha_rec, cod_cre=Credito.cod_cre).first()
            if CarCatHis == None:
                CarCatHis = justo.CARTE_CAT_HIS.objects.create(
                    oficina=Oficina, fecha=self.fecha_rec, cod_cre=Credito.cod_cre)
            ...
            CarCatHis.int_cau_res_per = (
                liq_cre.int_cau_fra+liq_cre.aju_ic_a_pag) if liq_cre.sal_cap_tot > 0 else 0
            CarCatHis.vr_gar_hip = Credito.val_gar_hip
            CarCatHis.conta_per = (
                liq_cre.cuo_pac - liq_cre.altura) if liq_cre.cuo_pac - liq_cre.altura else 36
        ...
        for row in CatPorNit:
            ...
            CatPorNits = justo.CARTE_CAT_HIS.objects.filter(
                oficina=Oficina, fecha=ifecha, nit=row['nit'])
            xSaldo_1 = xSaldo_2 = 0
            ...
            for CatPorNit in CatPorNits:
                ...
                CatPorNit.save()
        CarCatPers = justo.CARTE_CAT_HIS.objects.filter(
            oficina=Oficina, fecha=ifecha)
        for CarCatPer in CarCatPers:
            if CarCatPer.arrastre != 'A':
                PorProInd = justo.IMP_CON_CRE_INT.objects.filter(
                    cliente=Oficina.cliente, cod_imp=CarCatPer.cod_imp_con, categoria=CarCatPer.arrastre).first()
                if PorProInd == None:
                    xTasPro = 100
                else:
                    xTasPro = PorProInd.porcen_det
                CarCatPer.aporte = round(
                    CarCatPer.aporte/CarCatPer.saldo_2*CarCatPer.sal_cap if CarCatPer.saldo_2 > 0 else 0, 0)
                ...
                if CarCatPer.dias_mor > 0 and CarCatPer.plazo == 1:
                    xPorGarApo = 0
                xProvision = round(
                    (CarCatPer.sal_cap - (CarCatPer.vr_gar_hip*xPorGarHip))*xTasPro/100, 0)
                CarCatPer.pro_ind = xProvision if xProvision > 0 else 0
                CarCatPer.vr_gar_hip = round(
                    CarCatPer.vr_gar_hip*xPorGarHip, 0)
            else:
                if CarCatPer.saldo_2 == 0 and CarCatPer.saldo_1 > 0:
                    CarCatPer.aporte = round(
                        CarCatPer.aporte/CarCatPer.saldo_1*CarCatPer.sal_cap, 0)
                    CarCatPer.pro_ind = 0
                else:
                    CarCatPer.aporte = 0
                    CarCatPer.pro_ind = 0
...
#   Aqui se crea lA RECLASIFICACION UNIDA INICIANDO EN LA DEL Periodo Anterior
        CatInis = justo.CARTE_CAT_HIS.objects.filter(
            oficina=Oficina, fecha=self.fecha_ant_rec)
        for CatIni in CatInis:
            if CatIni.castigo == 'T':
                continue
            CateInt = justo.CATE_INTE.objects.filter(
                oficina=Oficina, fecha=self.fecha_rec, cod_cre=CatIni.cod_cre).first()
            if CateInt == None:
                CateInt = justo.CATE_INTE.objects.create(
                    oficina=Oficina, fecha=self.fecha_rec, cod_cre=CatIni.cod_cre)
            CateInt.tipo = '1'
...
#   continua cLA UNION con el periodo actual
        CatFins = justo.CARTE_CAT_HIS.objects.filter(
            oficina=Oficina, fecha=self.fecha_rec)
        for CatFin in CatFins:
            CateInt = justo.CATE_INTE.objects.filter(
                oficina=Oficina, fecha=self.fecha_rec, cod_cre=CatFin.cod_cre).first()
            if CateInt == None:
                CateInt = justo.CATE_INTE.objects.create(
                    oficina=Oficina, fecha=self.fecha_rec, cod_cre=CatFin.cod_cre)
                # 0 Si el credito no existia en el periodo NTERIOS
                CateInt.tipo = '0' if CatFin.sal_cap <= 0 else '1'
            else:
                # 2 Si termino en este periodo 3 si esta en ambos periodos
                CateInt.tipo = '3' if CatFin.sal_cap > 0 else '2'
...
#   Ahora se carga cod_imp y fec des a partir de los creditos
        CateInts = justo.CATE_INTE.objects.filter(
            oficina=Oficina, fecha=self.fecha_rec)
        for CateInt in CateInts:
            Cred = justo.CREDITOS.objects.filter(
                oficina=Oficina, cod_cre=CateInt.cod_cre).first()
...
#  aqui se recorre la cat del periodo para calcular reclasificacion del Capital
        ...
        CreRecs = justo.CATE_INTE.objects.filter(
            oficina=Oficina, fecha=self.fecha_rec)
        for CreRec in CreRecs:
            ...
            xArrIni = CreRec.arr_ini if CreRec.arr_ini > ' ' and CreRec.arr_ini < 'F' else (
                'E' if CreRec.arr_ini == 'F' else 'A')
            xArrFin = CreRec.arr_fin if CreRec.arr_fin > ' ' and CreRec.arr_fin < 'F' else (
                'E' if CreRec.arr_fin == 'F' else 'A')
            # sin cambios no son necesarios los ASIENTOS
            if CreRec.sal_cap_ini == CreRec.sal_cap_fin and xArrIni == xArrFin:
                continue
            ImpCon = justo.IMP_CON_CRE.objects.filter(
                cliente=Cliente, cod_imp=xCodImp).first()
#  Se hacen los asientos para que el saldo de la cuenta puente vuelva a cero
            if CreRec.sal_cap_ini < CreRec.sal_cap_fin:  # Se Desembolso en ese mes
                self.AsiConCap.actualizar(
                    ImpCon.kpte_cap, xNitEmpresa, 'CapPte', '', 0, CreRec.sal_cap_fin - CreRec.sal_cap_ini)
            else:  # Se hicieron pagos
                self.AsiConCap.actualizar(
                    ImpCon.kpte_cap, xNitEmpresa, 'CapPte', '', CreRec.sal_cap_ini - CreRec.sal_cap_fin, 0)
#  Deterioro general y general adicional
            xDifProGen = round(CreRec.sal_cap_ini*0.01, 0) - \
                round(CreRec.sal_cap_fin*0.01, 0)
            xDifProGenAdi = round(CreRec.sal_cap_ini*0.005, 0) - \
                round(CreRec.sal_cap_fin*0.005, 0)
            RegCapIni = justo.IMP_CON_CRE_INT.objects.filter(
                cliente=Cliente, cod_imp=xCodImp, categoria=xArrIni).first()
            RegCapFin = justo.IMP_CON_CRE_INT.objects.filter(
                cliente=Cliente, cod_imp=xCodImp, categoria=xArrFin).first()
            ...
            if CreRec.sal_cap_ini > 0:  # Se debe asegurar que el credito no se haya desembolsado ese periodo
                if xDifProGen + xDifProGenAdi > 0:  # se desembolsa el credito
                    self.AsiConCap.actualizar(
                        ImpCon.kdet_gen, CreRec.nit, CreRec.cod_cre, CreRec.cod_cre, 0, xDifProGen)
                    self.AsiConCap.actualizar(
                        ImpCon.kdet_gen_adi, CreRec.nit, CreRec.cod_cre, CreRec.cod_cre, 0, xDifProGenAdi)
                    self.AsiConCap.actualizar(
                        ImpCon.kdet_gen_gas, CreRec.nit, CreRec.cod_cre, CreRec.cod_cre, xDifProGen+xDifProGenAdi, 0)
                    if CreRec.sal_cap_ini > 0:  # Se Desembolso el credito en el periodo Anterior
                        self.AsiConCap.actualizar(
                            ImpCon.kdet_gen, CreRec.nit, CreRec.cod_cre, CreRec.cod_cre, xDifProGen, 0)
                        self.AsiConCap.actualizar(
                            ImpCon.kdet_gen_adi, CreRec.nit, CreRec.cod_cre, CreRec.cod_cre, xDifProGenAdi, 0)
                else:  # Debe haber saldo inicial ya que el desembolso pudo haber ocurrido ese mes
                    self.AsiConCap.actualizar(
                        ImpCon.kdet_gen, CreRec.nit, 'DetGen', CreRec.cod_cre, -xDifProGen, 0)
                    self.AsiConCap.actualizar(
                        ImpCon.kdet_gen_adi, CreRec.nit, 'DetGen', CreRec.cod_cre, -xDifProGenAdi, 0)
                    if CreRec.fec_des.year == self.fecha_rec.year:  # Puede Disminuir el Gasto por ser del mismo Año
                        self.AsiConCap.actualizar(
                            ImpCon.kdet_gen_gas, CreRec.nit, 'DetGenGas', CreRec.cod_cre, 0, xDifProGen+xDifProGenAdi)
                    else:  # Ingreso Recuperado por ser del año siguiente a la causacion
                        self.AsiConCap.actualizar(
                            ImpCon.kdet_gen_rec, CreRec.nit, 'DetGen', CreRec.cod_cre, 0, xDifProGen+xDifProGenAdi)
...
#  Se reclasifica el Capital  y se liquida  el Deteterioro individual
            if CreRec.sal_cap_ini > 0:  # se reversa el asientos de Capital y deterioro individual para generar el nuevo
                self.AsiConCap.actualizar(RegCapIni.kcapital, CreRec.nit, 'Capita_' +
                                          xArrIni, CreRec.cod_cre, 0, CreRec.sal_cap_ini)  # Se reversa capital
                self.AsiConCap.actualizar(RegCapIni.kdet_ind, CreRec.nit, 'DetInd_' +
                                          xArrIni, CreRec.cod_cre, xDetIni, 0)  # Se reversa Deterioro  Ind
            if CreRec.sal_cap_fin > 0:  # se Generan los nuevos asientos de Reclasificacion de Cpaital y deterioro Individual
                RegCapFin = justo.IMP_CON_CRE_INT.objects.filter(
                    cliente=Cliente, cod_imp=xCodImp, categoria=xArrFin).first()
                self.AsiConCap.actualizar(
                    RegCapFin.kcapital, CreRec.nit, 'Capita_'+xArrFin, CreRec.cod_cre, CreRec.sal_cap_fin, 0)
                self.AsiConCap.actualizar(
                    RegCapFin.kdet_ind, CreRec.nit, 'DetInd_'+xArrFin, CreRec.cod_cre, 0, xDetFin)
            xDifDetInd = xDetIni - xDetFin
            xDetIndGasAcu = CreRec.det_ind_gas_acu
            if xDifDetInd < 0:  # se incrementa el gasto
                self.AsiConCap.actualizar(
                    ImpCon.kdet_ind_gas, CreRec.nit, 'DetIndGas', CreRec.cod_cre, xDetFin, xDetIni)
                ...
                if xIng_x_cau < xDifDetInd:  # no alcanza con el ingreso recuperado pero se aplica lo que alcance
                    self.AsiConCap.actualizar(
                        ImpCon.kdet_ind_rec, CreRec.nit, 'DetIndGas', CreRec.cod_cre, 0, xIng_x_cau)
                    self.AsiConCap.actualizar(
                        ImpCon.kdet_ind_gas, CreRec.nit, 'DetIndGas', CreRec.cod_cre, 0, xDifDetInd-xIng_x_cau)
                    xDetIndGasAcu = xDetIndGasAcu - (xDifDetInd-xIng_x_cau)
                else:
                    self.AsiConCap.actualizar(
                        ImpCon.kdet_ind_rec, CreRec.nit, 'DetIndGas', CreRec.cod_cre, 0, xDifDetInd)
    ...
    def ReclaProv_interes(self, Oficina):
        ...
        CurCatHis = CateHist()
        CateIntes = justo.CATE_INTE.objects.filter(
            oficina=Oficina, fecha=self.fecha_rec)
        for CateInte in CateIntes:
            ...
            CateInte.int_cau_mes = CateInte.int_dia_fin + CateInte.int_cau_fin + \
                CateInte.int_pag - CateInte.int_dia_ini - CateInte.int_cau_ini
            ...
            CateInte.save()
            CreCatHiss = justo.CARTERA_CXC.objects.filter(
                oficina=Oficina, fecha=self.fecha_ant_rec, cod_cre=CateInte.cod_cre)
            for Reg in CreCatHiss:
                CurCatHis.actualizar(
                    Reg.cod_cre, Reg.fec_ref, Reg.categoria, Reg.valor)
            RegCat = justo.CARTE_CAT_HIS.objects.filter(
                oficina=Oficina, cod_cre=CateInte.cod_cre, fecha=self.fecha_ant_rec).first()
            if RegCat != None:
                if RegCat.sal_cat_int != 0:
                    CurCatHis.actualizar(
                        RegCat.cod_cre, self.fecha_ant_rec, RegCat.cat_int_mes, RegCat.sal_cat_int)
#   La rutina Importante
        print('     Comienza el ciclo ', datetime.now())
        justo.CARTERA_CXC.objects.filter(
            oficina=Oficina, fecha=self.fecha_rec).delete()
        xtc = 0
        CateIntes = justo.CATE_INTE.objects.filter(
            oficina=Oficina, fecha=self.fecha_rec)
        for Reg in CateIntes:
            ...
            xtx = xtc + 1
            # comienza debiendo termina debiendo (+)
            if xIni >= 0 and xFin >= 0 and xIni < xFin:
                xTipo = 1
            # comienza debiendo termina debiendo (-)
            elif xIni >= 0 and xFin >= 0 and xIni >= xFin:
                xTipo = 2
                ...
                xTipo = 6
            # comienza con saldo a favor termina con saldo a favor (-)
            elif xIni < 0 and xFin < 0 and xFin >= xIni:
                xTipo = 7
            # comienza con saldo a favor termina con saldo a favor (+)
            elif xIni < 0 and xFin < 0 and xFin < xIni:
                xTipo = 8
            ImpCon = justo.IMP_CON_CRE.objects.filter(
                cliente=Oficina.cliente, cod_imp=Reg.cod_imp).first()
            if ImpCon == None:
                ImpCon = justo.IMP_CON_CRE.objects.filter(
                    cliente=Oficina.cliente, cod_imp=Reg.cod_imp).first()
                if ImpCon == None:
                    print('Grave  ', Reg.cod_cre, '  Inicio ',
                          Reg.cod_imp, ' Final ', Reg.cod_imp)
            ImpConCat = justo.IMP_CON_CRE_INT.objects.filter(
                cliente=Oficina.cliente, cod_imp=Reg.cod_imp, categoria=xCatFin).first()
            if ImpConCat == None:
                ImpConCat = justo.IMP_CON_CRE_INT.objects.filter(
                    cliente=Oficina.cliente, cod_imp=Reg.cod_imp, categoria=xCatFin).first()

            CatAct = justo.CARTE_CAT_HIS.objects.filter(
                oficina=Oficina, fecha=self.fecha_rec, cod_cre=Reg.cod_cre).first()
            if Reg.cat_fin < 'C':  # Los Interese causados van a ingreso
                self.AsiRecInt.actualizar(
                    ImpCon.kcta_ingreso, xNit, 'Ingres', xCodCre, 0, Reg.int_cau_mes)
                zCredito = zCredito + Reg.int_cau_mes
                Reg.ingreso = Reg.ingreso - Reg.int_cau_mes
                xDifMes = Reg.int_cau_mes - Reg.int_pag_act - \
                    (-xIni if xTipo == 3 else (-xIni if xTipo > 6 else 0))
                xDifMes = xDifMes - \
                    (Reg.int_pag if Reg.int_pag < 0 and xTipo == 3 else 0)
                if xDifMes > 0:
                    self.AsiRecInt.actualizar(
                        ImpConCat.kcxc_ic, xNit, 'CxC', xCodCre, xDifMes, 0)
                ...
                if xIngCre > 0:
                    ...
                    self.AsiRecInt.actualizar(
                        ImpCon.kcta_ingreso, xNit, 'Ingres', xCodCre, 0, xIngCre)
                    zCredito = zCredito + xIngCre  # xc+CateInte.IPAct
                if Reg.int_cau_mes > xIngCre:
                    self.AsiRecInt.actualizar(
                        ImpConCat.kord_ic, xNit, 'CtaOrd', xCodCre, 0, Reg.int_cau_mes - xIngCre)
                    if Reg.cat_fin == 'C':
                        CatZ = 'W'
                    elif Reg.cat_fin == 'D':
                        CatZ = 'X'
                        ...
                        CatZ = 'Z'
                    self.AsiRecInt.actualizar(
                        ImpCon.kic_orden_i, xNit, 'CtaOrd', xCodCre, Reg.int_cau_mes - xIngCre, 0)
                ...
                if Reg.int_pag > 0:
                    self.AsiRecInt.actualizar(
                        ImpCon.kpte_ic, xNit, 'Ingres', xCodCre, Reg.int_pag, 0)
                    zDebito = zDebito + Reg.int_pag
                elif Reg.int_pag < 0:
                    self.AsiRecInt.actualizar(
                        ImpCon.kpte_ic, xNit, 'Ingres', xCodCre, 0, -Reg.int_pag)
                    zCredito = zCredito - Reg.int_pag

            if Reg.int_pag > 0:  # Aqui Afecta la Cta Pte
                self.AsiRecInt.actualizar(
                    ImpCon.kpte_ic, xNit, 'CtaPte', xCodCre, Reg.int_pag, 0)
                zDebito = zDebito + Reg.int_pag
            else:  # Pudo haber sido un pago negativo cuando es Interes Adelantado
                self.AsiRecInt.actualizar(
                    ImpCon.kpte_ic, xNit, 'CtaPte', xCodCre, 0, -Reg.int_pag)
                zCredito = zCredito - Reg.int_pag
            ...
            if xTipo < 3 or xTipo == 6:
                if Reg.int_pag_ant > 0 or (xTipo == 2 and Reg.ingreso > 0):
                    ...
                    RegHiss = [
                        regis for regis in CurCatHis.rows if regis.CodCre == Reg.cod_cre]
                        ...
                        if RegHis.Valor > 0:
                            ...
                            if xAboCxC > 0:
                                ...
                                if RegHis.Categoria < 'G':
                                    RegImpCatH = justo.IMP_CON_CRE_INT.objects.filter(
                                        cliente=Oficina.cliente, cod_imp=ImpCon.cod_imp, categoria=RegHis.Categoria).first()
                                    self.AsiRecInt.actualizar(
                                        RegImpCatH.kcxc_ic, xNit, 'CxC', xCodCre, 0, xAboCxC)
                                    ...
                                    self.AsiRecInt.actualizar(
                                        ImpCon.kcta_ingreso, xNit, 'IngRec ', xCodCre, 0, xAboCxC)
                                    zCredito = zCredito + xAboCxC
                                    if RegHis.Categoria == 'W':
                                        ...
                                        xCtaHom = 'F'
                                    RegImpCatH = justo.IMP_CON_CRE_INT.objects.filter(
                                        cliente=Oficina.cliente, cod_imp=ImpCon.cod_imp, categoria=xCtaHom).first()
                                    self.AsiRecInt.actualizar(
                                        RegImpCatH.kord_ic, xNit, 'IngRec', xCodCre, 0, xAboCxC)
                                    zCredito = zCredito + xAboCxC
                                    self.AsiRecInt.actualizar(
                                        ImpCon.kic_orden_i, xNit, 'IngRec', xCodCre, xAboCxC, 0)
                    ...
                    if xLimCxC > 0:
                        self.AsiRecInt.actualizar(
                            '51109501', xNit, 'Error', xCodCre, 0, xLimCxC)
                        zCredito = zCredito + xLimCxC
                if xTipo == 6:
                    self.AsiRecInt.actualizar(
                        '27200505', xNit, 'CxP', xCodCre, xLimCxC, Reg.int_pag_ade)
                    zCredito = zCredito + Reg.int_pag_ade
                    Reg.cue_por_pag = Reg.cue_por_pag - Reg.int_pag_ade
            if xTipo == 3:
                self.AsiRecInt.actualizar(
                    '27200505', xNit, 'CxP', xCodCre, -xIni, 0)
            ...
            if xTipo == 7 or xTipo == 8:
                ...
                if xDifDef > 0:
                    self.AsiRecInt.actualizar(
                        '27200505', xNit, 'CxP', xCodCre, xDifDef, 0)
                    zCredito = zCredito - xDifDef
                    Reg.cue_por_pag = Reg.cue_por_pag + xDifDef
                else:
                    self.AsiRecInt.actualizar(
                        '27200505', xNit, 'CxP', xCodCre, 0, -xDifDef)
            ...
            for RegHis in RegHiss:
                ...
                if RegHis.Valor > 0:
                    if RegHis.Categoria != xNueCat and RegHis.Categoria <= 'F':
                        ImpConCat1 = justo.IMP_CON_CRE_INT.objects.filter(
                            cliente=Oficina.cliente, cod_imp=ImpCon.cod_imp, categoria=RegHis.Categoria).first()
                        self.AsiRecInt.actualizar(
                            ImpConCat1.kcxc_ic, xNit, 'CxC', xCodCre, 0, RegHis.Valor)
                        zCredito = zCredito + RegHis.Valor
                        ImpConCat2 = justo.IMP_CON_CRE_INT.objects.filter(
                            cliente=Oficina.cliente, cod_imp=ImpCon.cod_imp, categoria=xNueCat).first()
                        self.AsiRecInt.actualizar(
                            ImpConCat2.kcxc_ic, xNit, 'CxC', xCodCre, RegHis.Valor, 0)
                    ...
                    if RegHis.Categoria > "F" and ((RegHis.Categoria == "W" and xNueCat != "C") or (RegHis.Categoria == "X" and xNueCat != 'D') or (RegHis.Categoria == 'Y' and xNueCat != 'E') or (RegHis.Categoria == 'Z' and xNueCat != 'F')):
                        ...
                        if xNueCat < 'C':
                            ...
                            self.AsiRecInt.actualizar(
                                ImpCon.kic_orden_i, xNit, 'CtaOrI', xCodCre, RegHis.Valor, 0)
                            zDebito = zDebito + RegHis.Valor
                            self.AsiRecInt.actualizar(
                                ImpCon.kcta_ingreso, xNit, 'IngRec', xCodCre, 0, RegHis.Valor)
                            zCredito = zCredito + RegHis.Valor
                            ImpConCat1 = justo.IMP_CON_CRE_INT.objects.filter(
                                cliente=Oficina.cliente, cod_imp=ImpCon.cod_imp, categoria=xNueCat).first()
                            self.AsiRecInt.actualizar(
                                ImpConCat1.kcxc_ic, xNit, 'CtaOrI', xCodCre, RegHis.Valor, 0)
                            zDebito = zDebito + RegHis.Valor
                            xNueCatSal = xNueCat
                        else:
                            ImpConCat1 = justo.IMP_CON_CRE_INT.objects.filter(
                                cliente=Oficina.cliente, cod_imp=ImpCon.cod_imp, categoria=xNueCat).first()
                            self.AsiRecInt.actualizar(
                                ImpConCat1.kord_ic, xNit, 'CxC', xCodCre, RegHis.Valor, 0)
                            zDebito = zDebito + RegHis.Valor
                            if Reg.cat_fin == 'C':
                                xNueCatSal = 'W'
                            if Reg.cat_fin == 'D':
            ...
            if zDebito != zCredito:
                if zDebito > zCredito:
                    self.AsiRecInt.actualizar(
                        '51109501', xNit, 'Error ', xCodCre, 0, zDebito - zCredito)
                else:
                    self.AsiRecInt.actualizar(
                        '51109501', xNit, 'Error ', xCodCre, -zDebito+zCredito, 0)
            #   Termino
            RegHiss = [
                regis for regis in CurCatHis.rows if regis.CodCre == Reg.cod_cre]
            for RegHis in RegHiss:
    ...
    def Llevar_a_historico(self, Oficina):
        ...
        for Credito in Creditos:
            ...
            if Credito.fec_des.year == self.fecha_rec.year and Credito.fec_des.month == self.fecha_rec.month:
                continue
            HisAnt = justo.CARTE_CAT_HIS.objects.filter(
                oficina=Oficina, fecha=self.fecha_ant_rec, cod_cre=Credito.cod_cre).first()
            if HisAnt == None:
                Credito.estado = 'H'
                Credito.save()
...
class perdida_esperada:
    ...
    def calculo_pi(self, iCodCre):
        ...
        self.arr_his.fill(0)
        Cred = justo.CREDITOS.objects.filter(
            oficina=self.Oficina, cod_cre=iCodCre).first()
        ...
        xANTIPRE2 = 1 if (
            Cred.fec_des - Cred.socio.fec_afi).days > 365*3 else 0
        xREESTRUCT = 0
        xAporte = 0
        ...
        CtasAho = justo.CTAS_AHORRO.objects.filter(
            oficina=self.Oficina, asociado=Cred.socio)
        ...
        for iMes in range(36):
            CreHis = justo.CARTE_CAT_HIS.objects.filter(
                oficina=self.Oficina, fecha=fec_cor, cod_cre=iCodCre).first()
            if CreHis != None:
        ...
        for iMes in range(36):
            ...
            if iMes < 12:
                ...
                NMORAS1_31_60 = NMORAS1_31_60 + \
                    (1 if self.arr_his[iMes] >=
                     31 and self.arr_his[iMes] <= 60 else 0)
            if iMes < 3:
                NMORAS3_31_60 = NMORAS3_31_60 + \
                    (1 if self.arr_his[iMes] >=
                     31 and self.arr_his[iMes] <= 60 else 0)

        xTC = xFE = xESIN = xFAMOR = xVALCUOTA = xVALPRES = xOCOOP = xFONAHO = xFONDPLAZO = xENTIDAD1 = 0
        HisPun = justo.PE_CARTE_HIS.objects.filter(
            oficina=self.Oficina, fecha=self.fecha, cod_cre=iCodCre).first()
        if HisPun == None:
            HisPun = justo.PE_CARTE_HIS.objects.create(
                oficina=self.Oficina, fecha=self.fecha, cod_cre=iCodCre)
        ...
        Mod = justo.PE_MODE_REFE.objects.filter(
            cliente=self.Oficina.cliente, modalidad=HisPun.modalidad).first()
        xz = 0
        xz = xz + Mod.constante + HisPun.val_ea*Mod.coe_ea + HisPun.val_ap * \
            Mod.coe_ap + HisPun.val_tc * Mod.coe_tc + HisPun.val_fe * Mod.coe_fe
        xz = xz + HisPun.val_famor*Mod.coe_famor + HisPun.val_valcuota*Mod.coe_valcuota + \
            HisPun.val_valpres*Mod.coe_valpres + HisPun.val_ocoop*Mod.coe_ocoop
        xz = xz + HisPun.val_fonaho*Mod.coe_fonaho + HisPun.val_fondplazo*Mod.coe_fondplazo + \
            HisPun.val_antipre1*HisPun.val_antipre1 + HisPun.val_coopcdat*Mod.coe_coopcdat
        xz = xz + HisPun.val_mora15*Mod.coe_mora15 + HisPun.val_mora1230*Mod.coe_mora1230 + \
            HisPun.val_mora1260*Mod.coe_mora1260 + HisPun.val_mora2430*Mod.coe_mora2430
        xz = xz + HisPun.val_mora2460*Mod.coe_mora2460 + HisPun.val_mora3615*Mod.coe_mora3615 + \
            HisPun.val_m1mora30*Mod.coe_m1mora30 + HisPun.val_m2mora30*Mod.coe_m2mora30
        xz = xz + HisPun.val_m2mora30*Mod.coe_m2mora30 + HisPun.val_m1mora30*Mod.coe_m1mora30 + \
            HisPun.val_m1mora30m3+Mod.coe_m1mora30m3 + HisPun.val_simmora*Mod.coe_sinmora
        xz = xz + HisPun.val_mortrim*Mod.coe_mortrim + HisPun.val_cuenaho*Mod.coe_cuenaho + \
            HisPun.val_cdat*Mod.coe_cdat + HisPun.val_entidad1*Mod.coe_entidad1
        xz = xz + HisPun.val_antipre2*Mod.coe_antipre2 + HisPun.val_vin2*Mod.coe_vin2 + \
            HisPun.val_entidad2*Mod.coe_entidad2 + HisPun.val_plazol*Mod.coe_plazol
        xz = xz + HisPun.val_nodo1*Mod.coe_nodo1 + HisPun.val_nodo4*Mod.coe_nodo4 + \
            HisPun.val_nodo5*Mod.coe_nodo5 + HisPun.val_nodo7*Mod.coe_nodo7
        xz = xz + HisPun.val_nodo8*Mod.coe_nodo8 + HisPun.val_nodo9*Mod.coe_nodo9 + \
            HisPun.val_salpres*Mod.coe_salpres + HisPun.val_reest*Mod.coe_reest
        ...
        CarCatHis = justo.CARTE_CAT_HIS.objects.filter(
            oficina=self.Oficina, fecha=self.fecha, cod_cre=HisPun.cod_cre).first()
        if CarCatHis == None:
            print(HisPun.cod_cre)
            return
        PePdi = justo.PE_PDI_RANGO.objects.filter(
            cliente=self.Oficina.cliente, garantia=Cred.tip_gar).first()
...
def init_recla():
    ...
    print('Fin Reclasificacion         ', datetime.now(),
          ReclaMes.fecha_ant_rec, '  Fecha Ant Rec', ReclaMes.fecha_rec)
    return
...
def initcre():
    ...
    for Credito in Creditos:
        ...
        if liq_cre.lista_mov == None:
            ...
            continue
        liq_cre.liq_al_dia()
        sal_cre = Sal_cre(liq_cre.cod_cre, Credito.estado, liq_cre.sal_cap_tot, liq_cre.sal_cap_dia,
                          liq_cre.sal_int_dia, liq_cre.int_mor_cau, (liq_cre.fecha_focal-liq_cre.fec_al_dia).days)
