...
class Crear(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse('listar_capital_creditos')
...
class ImprimirPDF(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        contabilizacion = IMP_CON_CRE.objects.all().order_by('cliente', 'cod_con')

        # Creamos un objeto HttpResponse con el tipo de contenido PDF
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'inline; filename="IMP_CON_CRE.pdf"'
        ...
        # p.drawString(80, 720, f"Cuenta Puente Capital: {dato.kpte_cap}")
        # p.drawString(80, 700, f"Provisión General Adicional: {dato.kdet_gen_adi}")
        # p.drawString(80, 680, f"Provisión General: {dato.kdet_gen}")
        # p.drawString(80, 660, f"Cuenta Gasto Povisión: {dato.kdet_gen_gas}")
        # p.drawString(80, 640, f"Cuenta Provisión Recuperada: {dato.kdet_gen_rec}")
        # p.drawString(80, 620, f"Cuenta Provisión Individual Gasto: {dato.kdet_ind_gas}")
        # p.drawString(80, 600, f"Cuenta Provisión Individual Recuperada: {dato.kdet_ind_rec}")
        # p.drawString(80, 580, f"Cuenta Descuento Pronto Pago IC: {dato.kdpp_ic}")
        # p.drawString(80, 560, f"Cuenta Puente IC: {dato.kpte_ic}")
        # p.drawString(80, 540, f"Cuenta Valorización: {dato.cta_val}")
        # p.drawString(80, 520, f"Cuenta Valorización: {dato.kcta_ingreso}")
        # p.drawString(80, 500, f"Cuenta de Orden IC: {dato.kic_orden_i}")

        datos_tabla = [["cliente", "cod_imp", "descripcion", "kpte_cap", "kdet_gen_adi", "kdet_gen","kdet_gen_gas", "kdet_gen_rec", "kdet_ind_gas", "kdet_ind_rec", "kdpp_ic", "kpte_ic", "cta_val", "kcta_ingreso", "kic_orden_i"]]


        for dato in contabilizacion:
            datos_tabla.append([dato.cliente, dato.cod_imp, dato.descripcion, dato.kpte_cap, dato.kdet_gen_adi, dato.kdet_gen, dato.kdet_gen_gas, dato.kdet_gen_rec, dato.kdet_ind_gas, dato.kdet_ind_rec, dato.kdpp_ic, dato.kpte_ic, dato.cta_val, dato.kcta_ingreso, dato.kic_orden_i])
...
class ImprimePDF(LoginRequiredMixin, View):
    def get(self, request, pk, *args, **kwargs):
        # Recupera los datos de la base de datos
        # Asegúrate de adaptar esto a tu modelo y consulta específicos
        dato = IMP_CON_CRE.objects.get(pk=pk)
...
class ExportarView(LoginRequiredMixin, View):
    ...
    def exportar_pdf(self):
        ...
        contabilizacion = IMP_CON_CRE.objects.all()
    ...
    def exportar_excel(self):
        ...
        headers = [field.name for field in IMP_CON_CRE._meta.fields]
