...
class Lista(LoginRequiredMixin, ListView):
    model = CREDITOS
    form = CrearForm
    template_name = 'lista_creditos.html'
...
class Detalles(LoginRequiredMixin, DetailView):
    model = CREDITOS
    form = CrearForm
    template_name = 'detalles_credito.html'
...
class Crear(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse('listar_creditos')
...
class Actualizar(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    form = CrearForm
    fields = '__all__'
    template_name = 'actualizar_credito.html'
...
class ImprimirPDF(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        creditos = CREDITOS.objects.all().order_by('cliente', 'cod_con')

        # Creamos un objeto HttpResponse con el tipo de contenido PDF
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'inline; filename="CREDITOS.pdf"'
        ...
        datos_tabla = [["linea_ahorro", "cod_imp", "descripcion",
                        "ctaafeact", "ctaafeina", "ctaafeint", "ctaretfue"]]
...
class ImprimePDF(LoginRequiredMixin, View):
    def get(self, request, pk, *args, **kwargs):
        # Recupera los datos de la base de datos
        # Asegúrate de adaptar esto a tu modelo y consulta específicos
        dato = CREDITOS.objects.get(pk=pk)
...
class ExportarView(LoginRequiredMixin, View):
    ...
    def exportar_excel(self):
        ...
        headers = [field.name for field in CREDITOS._meta.fields]
        for col_num, header in enumerate(headers, 1):
            sheet.cell(row=1, column=col_num, value=header)

        # Añade datos a la hoja de cálculo
        creditos = CREDITOS.objects.all()
        for row_num, data in enumerate(creditos, start=2):
